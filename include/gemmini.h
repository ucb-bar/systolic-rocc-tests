...
// config
// Note: is_gemv disables transposing A capability
#define gemmini_extended3_gemv_config_ex(dataflow, sys_act, sys_shift, sys_acc_scale, C_stride, A_stride, A_transpose, B_transpose, set_only_strides) \
    ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, ((uint64_t)acc_scale_t_to_acc_scale_t_bits((acc_scale_t)sys_acc_scale) << 32) | ((uint64_t)(A_stride) << 16) | (B_transpose << 9) | (1 << 8) | ((set_only_strides) << 7) | (1<<6) | ((sys_act) << 3) | ((dataflow) << 2) | CONFIG_EX, ((uint64_t)(C_stride) << 48) | (sys_shift), k_CONFIG); \
...
static void sp_tiled_matvec_os(const elem_t * A, const elem_t * B, const void * D, void * C,
        scale_t A_scale_factor, scale_t B_scale_factor, scale_acc_t D_scale_factor,
  ...
  const uint32_t A_sp_addr_start = 0;
  const uint32_t B_sp_addr_start = (DIM+2)*BANK_ROWS - K;
  const uint32_t D_sp_addr_start = 1 << (ADDR_LEN-1);
  const uint32_t C_sp_addr_start = B_sp_addr_start - I;
  ...
  for (size_t i = 0; i < I; i+=DIM) {
      ...
      for (size_t k = 0; k < K; k++) {
        ...
        // If we're not using a bias, then we want to overwrite what's in the
        // accumulator, rather than writing over it
        int no_bias_new_matrix = D != NULL && k == K-1;
        if (no_bias_new_matrix) {
          out_sp_addr &= ~(1 << (ADDR_LEN-2));
        }

        const size_t A_cols = DIM - (k == K - 1 ? pad_K : 0);
        const size_t A_rows = DIM - (i + DIM >= I ? pad_I : 0);
        const size_t B_cols = DIM - (k == K - 1 ? pad_K : 0);
        const size_t B_rows = 1;
        const size_t C_cols = DIM;
        const size_t C_rows = DIM - (i + DIM >= I ? pad_I : 0);
  ...
  if (C != NULL) {
    ...
    size_t mat_dim_I = I*DIM - pad_I;
    size_t full_moves = mat_dim_I / (DIM * DIM);
    size_t row_moves = mat_dim_I % (DIM * DIM) / DIM;
    size_t col_moves = mat_dim_I % DIM;

    void * C_dram_addr = (int8_t*)C;
    for (size_t i = 0; i < full_moves; i++) {
      const uint32_t C_sp_addr = C_sp_addr_start + i*DIM;
      gemmini_extended_mvout(C_dram_addr, C_sp_addr, DIM, DIM);
      C_dram_addr += C_row_stride*DIM*sizeof_C;
    }
    if (row_moves != 0) {
      const uint32_t C_sp_addr = C_sp_addr_start + full_moves*DIM;
      gemmini_extended_mvout(C_dram_addr, C_sp_addr, DIM, row_moves);
      C_dram_addr += row_moves*C_row_stride*sizeof_C;
    }
    if (col_moves != 0) {
      const uint32_t C_sp_addr = C_sp_addr_start + full_moves*DIM + row_moves;
      gemmini_extended_mvout(C_dram_addr, C_sp_addr, col_moves, 1);
    }
  ...
  for (size_t k = 0; k < K; k++) {
    ...
    for (size_t i = 0; i < I/DIM; i++) {
      ...
      if (C != NULL && k == K-1) {
        ...
        const uint32_t C_sp_addr = C_sp_addr_start + i*DIM;
        // void * const C_dram_addr = (int8_t*)C + (i*C_row_stride + rounded_j)*DIM*sizeof_C;
        void * const C_dram_addr = (int8_t*)C + (i*DIM*sizeof_C);
...
// The above code is performing calculations to determine the padded dimensions of a systolic array based on the input dimensions `dim_I`, `dim_J`, and `dim_K`. It is also calculating the number of tiles needed in each dimension based on tiling factors `tile_I`, `tile_J`, and `tile_K`.
