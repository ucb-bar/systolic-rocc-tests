...
void init_random(elem_t * buf, int len) {
    ...
    for (elem_t * ptr = buf; ptr < buf + len; ptr++) {
      ...
      *ptr = 1;
#else
      *ptr = NN_floatToHalf((rand() % 5) - 2.0); //(rand() % 5) - 2;
#endif
    }
}
...
int main() {
    ...
    tiled_conv_dw_auto(BATCH_SIZE, IN_ROW_DIM, IN_COL_DIM,
            ...
            NO_ACTIVATION, NN_floatToHalf(1.0), 1, 0, 0,
    ...
    bool success = vec_is_equal_fp32(&output_fp32[0][0][0][0], &output_mat_fp32[0][0][0][0], sizeof(output_fp32) / sizeof(float), 1e-6);
#endif
    ...
    if (!success) {
        ...
        for (int och = 0; och < CHANNELS; och++) {
            ...
            for (int wrow = 0; wrow < KERNEL_DIM; wrow++) {
                ...
                for (int wcol = 0; wcol < KERNEL_DIM; wcol++) {
                    printf(" ");
                    NN_printFloat(NN_halfToFloat(weights[och][wrow][wcol]), 3);
                }
                printf("\b],\n");
            }
            printf("\b],");
        ...
        for (int batch = 0; batch < BATCH_SIZE; batch++) {
            ...
            for (int orow = 0; orow < OUT_ROW_DIM; orow++) {
                ...
                for (int ocol = 0; ocol < OUT_COL_DIM; ocol++) {
                    ...
                    for (int och = 0; och < CHANNELS; och++) {
                        printf(" ");
                        NN_printFloat(NN_halfToFloat(output_mat[batch][orow][ocol][och]),3);
                    }
                    printf("\b],");
                }
                printf("\b],\n");
