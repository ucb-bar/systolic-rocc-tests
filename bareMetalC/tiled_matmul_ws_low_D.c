...
//         C_full[r][c] += NN_floatToHalf(NN_halfToFloat(C_full[r][c]) + NN_halfToFloat(A[r][k])*NN_halfToFloat(B[k][c])); //A[r][k]*B[k][c];
//     }
// }
...
//     for (size_t i = 0; i < MAT_DIM_I; ++i) {
//       for (size_t j = 0; j < MAT_DIM_J; ++j) {
//         full_D[i][j] = NN_floatToHalf(1) ;//NO_BIAS ? 0 : NN_floatToHalf(1);  //RAND % 2;
//       }
//     }
...
#define RAND rand()
    ...
    for (size_t i = 0; i < MAT_DIM_I; ++i) {
      for (size_t j = 0; j < MAT_DIM_J; ++j) {
        full_D[i][j] = NN_floatToHalf(1); //NO_BIAS ? 0 : RAND % 2;
      }
    }
